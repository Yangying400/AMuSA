...
def refine_low_cosine_exposure(
    ...
    mask = exposures_norm < filtering_threshold
    ...
    for i, sample_name in enumerate(common_samples):
        ...
        coeffs, _ = scipy_nnls(active_sig_profiles, sample_counts)
    ...
    exposures_refit_df = exposures_refit_df.round(0).astype(int)

    # =====================================================
    # 7. Save output
    # =====================================================
    out_file = os.path.join(output_dir, f"{model_type}_low_cosine_refined_exposure.csv")
    exposures_refit_df.to_csv(out_file)
    ...
    # =====================================================
    return {
        "refined_exposure": exposures_refit_df
    }