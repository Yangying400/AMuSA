...
def compute_cosine(gt, recon):
    cosine_values = {}

    common_samples = gt.columns.intersection(recon.columns)

    for s in common_samples:
        v1 = gt[s].astype(float).values
        v2 = recon[s].astype(float).values
        ...
        if norm1 == 0 or norm2 == 0:
            ...
            cosine_values[s] = float(np.dot(v1, v2) / (norm1 * norm2))
...
def estimate_exposure_from_predictions(
    ...
    # 6. Output path
    # =============================
    exposure_dir = os.path.join( output_dir,"results", model_type)
   ...
   # 8. Cosine QC (NEW)
   # =========================================================
    reconstructed = np.dot(signature_matrix.values, exposures_df.values)
    ...
    cosine_values = compute_cosine(gt_df, recon_df)

    low_cosine_samples = [
        s for s, v in cosine_values.items()
        if v < cosine_threshold
    ]

    print(f"[Cosine QC] low-confidence samples: {len(low_cosine_samples)}")
    # =============================
    # 9. Save cosine results
    # =============================
    ...
    low_catalog = gt_df[low_cosine_samples]

    low_catalog_file = os.path.join(
        exposure_dir,
        f"{model_type}_low_cosine_catalog.csv"
    )

    low_catalog.to_csv(low_catalog_file)

    print(f"[Cosine QC] low-cosine catalog saved: {low_catalog_file}")
    ...
    "low_cosine_catalog_file": low_catalog_file,
...
def main():
    ...
    parser.add_argument("--cosine_threshold",type=float,default=0.9,help="Threshold for cosine similarity QC (default: 0.9)"
)
    # Epsilon (NEW)
    parser.add_argument(
        "--epsilon",
